...
public class MpscSingleThreadExecutor implements SingleThreadExecutor {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(MpscSingleThreadExecutor.class);
    ...
    private static final AtomicIntegerFieldUpdater<MpscSingleThreadExecutor> _STATE_UPDATER = AtomicIntegerFieldUpdater
    ...
    public MpscSingleThreadExecutor(int maxPendingTasks, ThreadFactory threadFactory) {
        this(maxPendingTasks, threadFactory, RejectedExecutionHandler.REJECT);
    }
    ...
    public boolean shutdownGracefully(long timeout, TimeUnit unit) {
        ...
        for (; ; ) {
            ...
            if (_STATE_UPDATER.compareAndSet(this, oldState, newState)) {
                break;
            }
        }
    ...
    protected Queue<Runnable> newTaskQueue(final int maxPendingTasks) {
        return maxPendingTasks == Integer.MAX_VALUE ? QueueUtil.newMpscQueue() : QueueUtil.newMpscQueue(maxPendingTasks);
    }
    ...
    private void startWorker() {
        ...
        if (_STATE_UPDATER.compareAndSet(this, ST_NOT_STARTED, ST_STARTED)) {
            try {
                doStartWorker();
            } catch (final Throwable t) {
    ...
    private void doStartWorker() {
        this.executor.execute(() -> {
            ...
            try {
                ...
                LOGGER.warn("Unexpected exception from executor: ", t);
            } finally {
                for (; ; ) {
                    int oldState = MpscSingleThreadExecutor.this.state;
                    if (oldState >= ST_SHUTDOWN || _STATE_UPDATER.compareAndSet(MpscSingleThreadExecutor.this, oldState, ST_SHUTDOWN)) {
                        break;
                    }
                }
    ...
    private class Worker implements Runnable {
        ...
        private void runTask(final Runnable task) {
            try {
                ...
                LOGGER.warn("Caught an unknown error while executing a task", t);
            }
        }

 ...
 * code from sofa-jraft
 */
public final class JPacificaServiceLoader<S> implements Iterable<S> {

    private static final Logger LOG = LoggerFactory.getLogger(JPacificaServiceLoader.class);
    ...
    public static <S> JPacificaServiceLoader<S> load(final Class<S> service) {
        return JPacificaServiceLoader.load(service, Thread.currentThread().getContextClassLoader());
    }

    public static <S> JPacificaServiceLoader<S> load(final Class<S> service, final ClassLoader loader) {
        return new JPacificaServiceLoader<>(service, loader);
    }
    ...
    private JPacificaServiceLoader(final Class<S> service, final ClassLoader loader) {
    ...
    private int parseLine(final Class<?> service, final URL u, final BufferedReader r, final int lc, final List<String> names) throws IOException, ServiceConfigurationError {
    ...
    private Iterator<String> parse(final Class<?> service, final URL url) {
        ...
        try (final InputStream in = url.openStream(); final BufferedReader r = new BufferedReader(new InputStreamReader(in, StandardCharsets.UTF_8))) {
            int lc = 1;
    ...
    public Iterator<S> iterator() {
        return new Iterator<S>() {
            ...
            final Iterator<Map.Entry<String, S>> knownProviders = JPacificaServiceLoader.this.providers.entrySet().iterator();

            @Override
            public boolean hasNext() {
                return this.knownProviders.hasNext() || JPacificaServiceLoader.this.lookupIterator.hasNext();
            }
            ...
            public S next() {
                ...
                final Class<S> cls = JPacificaServiceLoader.this.lookupIterator.next();
                return newProvider(cls);
            }
    ...
    public Iterator<Class<S>> classIterator() {
        return new Iterator<Class<S>>() {
            ...
            public Class<S> next() {
                ...
                return JPacificaServiceLoader.this.lookupIterator.next();
            }
    ...
    public String toString() {
        return "com.trs.pacifica.spi.JPacificaServiceLoader[" + this.service.getName() + "]";
    }
}

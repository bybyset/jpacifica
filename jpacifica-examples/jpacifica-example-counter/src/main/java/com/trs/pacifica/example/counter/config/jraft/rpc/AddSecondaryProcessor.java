...
public class AddSecondaryProcessor implements RpcProcessor<MetaReplicaRpc.AddSecondaryRequest> {


    private final MetaReplicaService metaReplicaService;

    public AddSecondaryProcessor(MetaReplicaService metaReplicaService) {
        this.metaReplicaService = metaReplicaService;
    }


    @Override
    public void handleRequest(RpcContext rpcCtx, MetaReplicaRpc.AddSecondaryRequest request) {
        RpcMetaReplicaClosure<Boolean> closure = new RpcMetaReplicaClosure<Boolean>(rpcCtx) {
            ...
            public Message buildRpcResponse(Boolean result) {
                return MetaReplicaRpc.AddSecondaryResponse.newBuilder().setSuccess(result).build();
            }
        };
        try {
            ...
            this.metaReplicaService.addSecondary(secondary, version, closure);
        } catch (Throwable e) {
    ...
    public String interest() {
        return MetaReplicaRpc.AddSecondaryRequest.class.getName();
    }
}
